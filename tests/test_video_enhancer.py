...
class TestVideoResolutionEnhancer(unittest.TestCase):
    ...
    def test_enhance_to_full_hd_916(self):
        """Verifica se um vídeo de baixa resolução (480x270) é convertido para 1080x1920 Full HD com nitidez."""
        vid_low = self._create_dummy_video(width=480, height=270, duration=1.0)
        vid_hd = os.path.join(self.test_dir, "output_hd.mp4")

        success, msg = self.enhancer.enhance_clip(vid_low, vid_hd, sharpen_strength=0.8, denoise=True)
