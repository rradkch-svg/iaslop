...
def sig_handler(sig, frame):
    ...
    watchdog_logger.info("🛑 [Watchdog] Sinal de interrupção recebido. Encerrando...")
    RUNNING = False
...
def is_generator_running() -> Tuple[bool, Optional[int], str]:
    """
    Verifica se o gerador (auto_pipeline.py) já está ativo.
        ...
        try:
            ...
            if test_handle:
                try:
                    ...
                    pass
            return True, recorded_pid, f"Instância ativa detectada com lock (PID: {recorded_pid})"
    else:
        import fcntl
        test_handle = None
        try:
...
def find_python_executable() -> str:
    """Encontra o interpretador Python adequado no sistema."""
    candidates = [
        sys.executable,
        "py -3.11",
        "py",
...
def spawn_generator_new_window():
    """Abre uma nova janela de terminal executando o gerador."""
...
def run_continuous_watchdog(poll_interval: int = 5):
    ...
    print("=" * 75)
    print("🛡️  MINUTO INEXPLICÁVEL - WATCHDOG SUPERVISOR DE ALTA RESILIÊNCIA")
    print("=" * 75)
...
def main():
    parser = argparse.ArgumentParser(description="Watchdog Supervisor do Minuto Inexplicável Studio")
    parser.add_argument("--check-only", action="store_true", help="Apenas checa se está rodando e retorna exit code")
    parser.add_argument("--check-and-spawn", action="store_true", help="Checa se está rodando; se fechado, abre em nova janela")
